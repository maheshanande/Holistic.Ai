...
import com.example.holisticai.ml.ModelType
import com.example.holisticai.ml.MoveNet
...
import com.example.holisticai.ml.Type
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch
import com.example.holisticai.camera.CameraSource
import com.example.holisticai.data.Device
import com.example.holisticai.ml.PoseNet
...
class sessionView : AppCompatActivity() {
    ...
    private val yogaLabels = listOf("chair", "cobra", "dog", "tree", "warrior")
    ...
    // Create a HashSet to store the corrected pose labels
    private lateinit var textToSpeech: TextToSpeech
    private var isSessionCompleted = false
    val correctedPoseLabels = HashSet<String>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        poseImageView = findViewById(R.id.yogaImage)
        ...
        textToSpeech = TextToSpeech(this) { status ->
            if (status == TextToSpeech.SUCCESS) {
                ...
                // TextToSpeech initialization failed, handle accordingly
            }
        }
                ...
                cameraSource =
                    CameraSource(surfaceView, object : CameraSource.CameraSourceListener {
                        ...
                        override fun onDetectedInfo(personScore: Float?, poseLabels: List<Pair<String, Float>>?) {
                            if (personScore != null && personScore >= 0.7 && poseLabels != null) {
                                if (currentImageIndex >= imageArray.size) {
                                    speakVoiceCommand("Well done! You have completed the session.")
                                    showToast("Great, you have completed all the images")
                                    poseImageView.visibility = View.INVISIBLE
                                    return
                                }

                                // Display the current image using the resource ID
                                val currentImageResourceId = imageArray[currentImageIndex]
                                runOnUiThread {
                                    poseImageView.setImageResource(currentImageResourceId)
                                    poseImageView.visibility = View.VISIBLE
                                }

                                val currentYogaLabel = yogaLabels[currentImageIndex]

                                // Check if the pose label is already corrected
                                if (correctedPoseLabels.contains(currentYogaLabel)) {
                                    // Move to the next image and increment the currentImageIndex
                                    currentImageIndex++
                                } else {
                                    var isPoseCorrected = false // Flag to track if the pose label is corrected

                                    for (poseLabelPair in poseLabels) {
                                        val (poseLabel, accuracy) = poseLabelPair

                                        // Check if the pose label matches the current yoga label
                                        if (poseLabel.equals(currentYogaLabel, ignoreCase = true)) {
                                            // Check if the accuracy is above the threshold
                                            if (accuracy > 0.95) {
                                                showToast("Pose is Correct: $poseLabel")
                                                correctedPoseLabels.add(currentYogaLabel)
                                                speakVoiceCommand("Pose is correct. Well done!")
                                                isPoseCorrected = true
                                                break
                                            }
                                        }
                                    }
                                    ...
                                    if (isPoseCorrected) {
                                        ...
                                        // Handle missed pose or continue checking in the next frame
                                    }
                                }
                                ...
                                // Handle the case where personScore is null or below threshold, or poseLabels is null
                            }
                        }
    ...
    private fun speakVoiceCommand(text: String) {
